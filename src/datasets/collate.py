...
def bss_collate_fn(dataset_items: list[dict]):
    ...
    """
    audio_parts = ["s1", "s2", "mix"] if "s1" in dataset_items[0] else ["mix"]
    video_parts = ["s1", "s2"]

    audios = {part: [] for part in audio_parts}
    videos = {part: [] for part in video_parts}
    embs = {part: [] for part in video_parts}
    audio_paths = {part: [] for part in audio_parts}
    video_paths = {part: [] for part in video_parts}
    emb_paths = {part: [] for part in video_parts}
