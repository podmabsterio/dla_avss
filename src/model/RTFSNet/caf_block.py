...
class CAFBlock(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            video (torch.Tensor):
                Video feature map of shape (B, C_v, T_v, F_v), where:
                    C_v — number of video channels (chan_video)
                    T_v, F_v — video time & spatial dims (may differ from audio)
        ...
        att = self.attention_conv(video)
        att = att.reshape(batch_size, self.in_chan_audio, 1, -1)
        att = att.mean(2, keepdim=False).view(batch_size, self.in_chan_audio, -1)
        att = F.interpolate(torch.softmax(att, -1), size=time_steps, mode="nearest")
        f1 = att.unsqueeze(-1) * a_v

        video_resized = self.resize_conv(video)
